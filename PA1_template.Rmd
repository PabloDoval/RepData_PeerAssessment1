...
dataNonNA <- data[complete.cases(data), ]
stepsPerInterval <- group_by(dataNonNA, date) %>% summarise(steps = mean(steps))
maximumStepsPerInterval <- max(stepsPerInterval$steps)
intervalOfMaximumSteps <- stepsPerInterval[stepsPerInterval$steps==max(maximumStepsPerInterval), 1]
```

According to this analysis, the maximum number of steps per interval, in average across all days, is **`r format(maximumStepsPerInterval, digits=2, nsmall=2)`**, which is on the interval **`r format(intervalOfMaximumSteps, digits=2, nsmall=2)`**.
